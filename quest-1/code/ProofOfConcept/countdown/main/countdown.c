...
#define TIMER_HOURS           1     // Timer hours unit, change to change feeding interval
#define TIMER_MINUTES         30    // Timer minutes unit, change to change feeding interval
...
static void Alnum_Disp(){
  ...
  // Write to disp in HHMM format
    sprintf(disp, "%2d%2d", (sec-(sec%(60*60)))/(60*60), (sec%(60*60))/60);
...
static void timer_evt_task(void *arg) {
    while (1) {
        ...
        // Increment / Wraparound when triggered
        if (evt.flag == 1) {
            if(sec == 0)
